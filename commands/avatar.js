...
module.exports = {
    ...
    async runInteraction(client, interaction) {
        ...
        const [results] = await db.query(`SELECT * FROM HeroicAcademy WHERE prefix = ${db.escape(prefix)} AND discordid = ${db.escape(interaction.user.id)}`)
        ...
        collector.on('collect', async collected => {
            ...
            db.query(`UPDATE HeroicAcademy SET iconURL = '${URL}' WHERE prefix = ${db.escape(prefix)} AND discordid = ${db.escape(interaction.user.id)}`)
