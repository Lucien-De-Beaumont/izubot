...
module.exports = {
    ...
    async runInteraction(client, interaction) {
        ...
        db.query(`UPDATE HeroicAcademy SET ${db.escape(option).slice(1, db.escape(option).length - 1)} = ${db.escape(newParameter)} WHERE discordid = ${db.escape(interaction.user.id)} AND prefix=${db.escape(prefix)}`)
