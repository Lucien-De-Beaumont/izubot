...
module.exports = {
    ...
    async runInteraction(client, interaction) {
        ...
        const [results0] = await db.query(`SELECT * FROM HeroicAcademy WHERE discordid=${db.escape(interaction.guild.members.cache.find(member => member.displayAvatarURL() == message.embeds[0].thumbnail.url).id)} ORDER BY nom ASC LIMIT 5 OFFSET ${((numberPages - 1) * 5)}`);
