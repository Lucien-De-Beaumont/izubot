...
module.exports = {
    ...
    async execute(client, reaction, user) {
        ...
        let wh
        let whOwnerID
        const [results] = await db.query(`SELECT * FROM HeroicAcademy WHERE nom=${db.escape(reaction.message.author.username)}`)
        if (results.length > 1) {
        ...
        if (whOwnerID && whOwnerID.length) {
            query = `SELECT * FROM HeroicAcademy WHERE nom=${db.escape(reaction.message.author.username)} AND discordid = ${db.escape(whOwnerID)}`
        } else {
            query = `SELECT * FROM HeroicAcademy WHERE nom=${db.escape(reaction.message.author.username)}`
        }
        if (!results[0]) { return }
